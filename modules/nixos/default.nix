      ...
      mkIf (cfg.linker != null)
      {
        systemd.targets.hjem = {
          ...
          in
            ...
            ++ (optionals (disabledUsers != {}) ["hjem-cleanup.service"]);
        };
        ...
        in {
          ...
          "hjem-activate@" = {
            ...
            in ''
              ...
              if [ ! -f /var/lib/hjem/manifest-$1.json ]; then
                ${linker} ${linkerOpts} activate $new_manifest
                exit 0
              fi

              ${linker} ${linkerOpts} diff $new_manifest /var/lib/hjem/manifest-$1.json
            '';
          };
          ...
          "hjem-copy@" = {
            ...
            */
            script = ''
              ...
              if ! cp $new_manifest /var/lib/hjem; then
                ...
                cp $new_manifest /var/lib/hjem
              fi
            '';
          };

          hjem-cleanup = mkIf (disabledUsers != {}) {
            description = "Cleanup disabled users manifests";
            serviceConfig.Type = "oneshot";
            after = ["hjem.target"];
            unitConfig.RefuseManualStart = false;
            script = "rm /var/lib/hjem/manifest-{${concatStringsSep "," (attrNames disabledUsers)}}.json";
          };
        };
      }
    )
